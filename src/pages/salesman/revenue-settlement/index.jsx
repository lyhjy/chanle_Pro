  ...
  constructor(props){
    ...
    this.state = {
      ...
      columns: [
        ...
        {title: '实际营收/元',dataIndex: 'realMoney',key: 'realMoney',hideInSearch: true,align: 'center'},
        {title: '审核记录',align: 'center',render: (_,recode) => <a onClick={() => this.viewAudit({ id: recode.id })}>查看</a>},
        {
          title: '操作', dataIndex: 'option', valueType: 'option', align: 'center', render: (_,recode) => {
            const { audit } = recode;
            const res = audit == 0 ? <a onClick={() => history.push({pathname: '/salesman/revenue-settlement/edit',state: {orderNo: recode.orderNo,id: recode.id}})}>修改</a> : <span>已提交</span>;
            return res;
          }
        }],
      auditColumns: [{
